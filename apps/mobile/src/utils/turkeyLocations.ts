...
export const turkeyLocations: City[] = [
  ...
  {
    name: 'Ankara',
    districts: ['Akyurt', 'Altındağ', 'Ayaş', 'Bala', 'Beypazarı', 'Çamlıdere', 'Çankaya', 'Çubuk', 'Elmadağ', 'Etimesgut', 'Evren', 'Gölbaşı', 'Güdül', 'Haymana', 'Kalecik', 'Kahramankazan', 'Keçiören', 'Kızılcahamam', 'Mamak', 'Nallıhan', 'Polatlı', 'Pursaklar', 'Sincan', 'Şereflikoçhisar', 'Yenimahalle']
  },
  {
    name: 'Antalya',
...
export const getCityNames = (): string[] => {
  return turkeyLocations.map(city => city.name);
};

// Get districts for a specific city
export const getDistrictsForCity = (cityName: string): string[] => {
  const city = turkeyLocations.find(c => c.name === cityName);
...
export const searchDistricts = (cityName: string, query: string): string[] => {
  const districts = getDistrictsForCity(cityName);
  const normalizedQuery = query.toLowerCase().trim();
  return districts.filter(district => district.toLowerCase().includes(normalizedQuery));
};
